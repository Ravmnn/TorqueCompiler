...
You can optionally omit the variable type by using `let` instead, making the compiler to automatically infer the type from the initial value. This feature is only allowed for function-scope variables (those that aren't fields, parameters, etc).
