...
public class IRGenerator : IBoundStatementProcessor, IBoundExpressionProcessor<IRExpressionResult>, IBoundDeclarationProcessor,
    IImportableProcessor
{
    ...
    public IRExpressionResult Process(BoundExpression expression)
        => expression.Process(this);


    public IReadOnlyList<IRExpressionResult> ProcessAll(IReadOnlyList<BoundExpression> expressions)
        => expressions.Select(Process).ToArray();




    public IRExpressionResult ProcessLiteral(BoundLiteralExpression expression)
    {
    ...
    public IRExpressionResult ProcessBinary(BoundBinaryExpression expression)
    {
    ...
    public IRExpressionResult ProcessUnary(BoundUnaryExpression expression)
    {
    ...
    public IRExpressionResult ProcessGrouping(BoundGroupingExpression expression)
        => Process(expression.Expression);




    public IRExpressionResult ProcessComparison(BoundComparisonExpression expression)
    {
        var left = EnsureValue(Process(expression.Left)).Value;
        var right = EnsureValue(Process(expression.Right)).Value;
    ...
    public IRExpressionResult ProcessEquality(BoundEqualityExpression expression)
    {
        var left = EnsureValue(Process(expression.Left)).Value;
        var right = EnsureValue(Process(expression.Right)).Value;
    ...
    public IRExpressionResult ProcessLogic(BoundLogicExpression expression)
    {
    ...
    public IRExpressionResult ProcessSymbol(BoundSymbolExpression expression)
    {
    ...
    public IRExpressionResult ProcessAddress(BoundAddressExpression expression)
    {
    ...
    public IRExpressionResult ProcessAssignment(BoundAssignmentExpression expression)
    {
    ...
    public IRExpressionResult ProcessPointerAccess(BoundPointerAccessExpression expression)
    {
    ...
    public IRExpressionResult ProcessCall(BoundCallExpression expression)
    {
    ...
    public IRExpressionResult ProcessCast(BoundCastExpression expression)
    {
        var value = EnsureValue(Process(expression.Value)).Value;
    ...
    public IRExpressionResult ProcessImplicitCast(BoundImplicitCastExpression expression)
    {
        var value = EnsureValue(Process(expression.Value)).Value;
    ...
    public IRExpressionResult ProcessArray(BoundArrayExpression expression)
    {
    ...
    public IRExpressionResult ProcessIndexing(BoundIndexingExpression expression)
    {
    ...
    public IRExpressionResult ProcessDefault(BoundDefaultExpression expression)
    {
        var llvmType = TypeBuilder.Process(expression.Type);
    ...
    public IRExpressionResult ProcessStruct(BoundStructExpression expression)
    {
    ...
    private IRExpressionResult IndexStruct(LLVMValueRef structAddress, StructType structType, string memberName)
    {
        var llvmStructType = TypeBuilder.Process(structType);
    ...
    private IRExpressionResult IndexPointer(LLVMTypeRef addressElementType, LLVMValueRef address, LLVMValueRef index)
        => Index(addressElementType, address, index);


    private IRExpressionResult IndexArray(LLVMTypeRef addressElementType, LLVMValueRef address, LLVMValueRef index)
        => Index(addressElementType, address, index, false);


    private IRExpressionResult Index(LLVMTypeRef addressElementType, LLVMValueRef address, LLVMValueRef index, bool scalar = true)
    {
    ...
    {
        var typeBuilder = new IRTypeBuilder();
        var llvmType = typeBuilder.Process(type);
    ...
    private IRExpressionResult Address(LLVMValueRef address, LLVMTypeRef type)
        => new IRExpressionResult(address, type, true);


    private IRExpressionResult Value(LLVMValueRef value, LLVMTypeRef type)
        => new IRExpressionResult(value, type, false);


    private IRExpressionResult EnsureAddress(IRExpressionResult expression)
    {
    ...
    private IRExpressionResult EnsureValue(IRExpressionResult expression)
    {
    ...
    private static void TryCatchControlException(Action action)
    {
        ...
        catch (IRGeneratorControlException)
        {}
    }
